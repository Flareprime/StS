...
//
// HOW IT WORKS
// The question screen has an <svg> with two <g> (group) elements:
...
//   Option A (separate image per expression): instead of swapping
//   innerHTML, show/hide an <img> per expression, e.g.
//     document.getElementById('avatar-thinking').style.display =
//       (expr === 'thinking') ? 'block' : 'none';
//   Option B (single animated sprite): just change the class on one
//   <img> element based on expr.
//
// ADDING A NEW EXPRESSION
// 1. Add a new key to FACE below with the SVG markup for the face.
// 2. Add a matching key (same name) to ARMS with the arm markup.
// 3. Call setAvatarExpression('yourNewKey') from the activity's JS.
...
const FACE = {
  ...
  // Extra: gold circles on the cheeks for a rosy/blushing look.
  happy: `
...
const ARMS = {
  ...
  // One arm relaxed at the side, the other bent up toward the chin -
  // the classic "thinking" pose.
  thinking: `
  ...
  // Both arms up with hands raised higher - a "whoa!" gesture.
  // Not a defensive/flinching pose - keep it reading as surprised, not scared.
  surprised: `
...
// 'thinking' instead of throwing - a typo here shouldn't crash the
// activity.
//
